...
- (instancetype)init{
    ...
    if (self) {
        ...
        _maxHeight = 110;
        _activity = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleLarge];
        [_backView addSubview:_activity];
        
        _textLabel = [[UILabel alloc] init];
        _textLabel.textColor = [UIColor whiteColor];
        _textLabel.font = [UIFont systemFontOfSize:kFont_Size_15];
        ...
        hideTextLabel.font = [UIFont systemFontOfSize:kFont_Size_15];
...
- (void)showActivityViewWithText:(NSString *)text superView:(UIView *)superView{
    ...
    CGFloat textWidth = [CommUtls returnWidthWithLabel:text frontSize:kContentSize maxSize:CGSizeMake(_maxHeight, 50)];
    NSInteger backViewWidth = MIN(textWidth + kActivityEdgeWidth * 2, _maxWidth);
    _backView.alpha = 1.0;
    if ([text length] > 0) {
